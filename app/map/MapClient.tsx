...
function MapClientContent() {
  ...
  return (
  /* 1. MAIN CONTAINER: Locked to viewport with no scroll */
  <div className="fixed inset-0 h-screen w-screen overflow-hidden bg-slate-100 flex flex-col">
    ...
    <div className="absolute top-0 left-0 right-0 z-[1001] p-4 pt-6 flex flex-col gap-3 pointer-events-none">
      ...
      {/* Main Control Bar: Centered Toggles + Right Profile */}
        ...
        <div className="col-span-3 pointer-events-auto flex justify-center">
          <div className="w-full max-w-[240px] bg-white/70 backdrop-blur-lg p-1 rounded-2xl shadow-2xl border border-white/40 flex gap-1">
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-[10px] font-black tracking-widest transition-all truncate px-1 ${
                !isLive ? "bg-blue-600 text-white shadow-md" : "text-slate-500"
              }`}
            >
              BROWSE
            </button>
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-[10px] font-black tracking-widest transition-all truncate px-1 ${
                isLive ? "bg-red-600 text-white shadow-md animate-pulse" : "text-slate-500"
              }`}
            >
              LIVE
            </button>
      ...
      {/* Dropdown - Sits under the control bar when in Browse */}
    ...
    {/* 4. BOTTOM OVERLAY: Action Buttons & Slim Info Dock */}
    {/* bottom-24 buffer for the RootLayout bottom-nav */}
    <div className="absolute bottom-24 left-4 right-4 z-[1002] flex flex-col pointer-events-none gap-3">
      
      {/* Small Action Buttons (Floating) */}
      {!isLive && activeRoute && (
        <div className="flex justify-between items-end w-full pointer-events-auto px-1">
          <div className="flex gap-2">
            <button onClick={shareRoute} className="w-10 h-10 bg-white/80 backdrop-blur-lg text-slate-600 rounded-xl shadow-xl border border-white/40 flex items-center justify-center active:scale-90 transition-all">
              <span className="material-symbols-rounded !text-xl">share</span>
            </button>
            {activeRoute?.createdBy === auth.currentUser?.uid && (
              <button onClick={() => deleteRoute(activeRoute.id)} className="w-10 h-10 bg-white/80 backdrop-blur-lg text-red-500 rounded-xl shadow-xl border border-white/40 flex items-center justify-center active:scale-90 transition-all">
                <span className="material-symbols-rounded !text-xl">delete</span>
              </button>
            )}
          </div>
          <button onClick={() => router.push("/routes/create")} className="w-12 h-12 bg-blue-600 text-white rounded-xl shadow-2xl flex items-center justify-center active:scale-90 transition-all">
      ...
      <div className="pointer-events-auto bg-white/90 backdrop-blur-xl px-5 py-3 rounded-2xl shadow-2xl border border-white/40">
        <div className="flex items-center justify-between gap-4">
          <div className="flex flex-col min-w-0">
            ...
            <h2 className="text-sm font-black italic uppercase text-slate-900 tracking-tight leading-none truncate">
              {activeRoute?.name || "Select Route"}
            </h2>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 truncate">
              {activeRoute?.neighborhood || "Portland"}
            </p>
          </div>
